...
int main( int argc, char** argv ) {
   ...
   if (argc < 5) {
      Log::crit("Insuficientes parametros para robot de carga, se esperaba (id, path_trasbordo, path_cinta_ppal id)\n");
      exit(1);
   }
   ...
   for(;;) {
      ...
      Log::info("Nuevo vuelo de trasbordo %i -> %i", trasbordo.vuelo_origen.num_vuelo, trasbordo.vuelo_destino.num_vuelo);

      sleep(rand() % SLEEP_TRASBORDO);

      Log::info("Sacando equipajes %i -> %i", trasbordo.vuelo_origen.num_vuelo, trasbordo.vuelo_destino.num_vuelo);

      std::vector<Equipaje> equipajes = equipajes_para(trasbordo);
      
      for(std::vector<Equipaje>::iterator equipaje = equipajes.begin(); equipaje < equipajes.end(); equipaje++) {
         sleep(rand() % SLEEP_TRASBORDO);
      ...
      Log::info("Terminé con los equipajes %i -> %i", trasbordo.vuelo_origen.num_vuelo, trasbordo.vuelo_destino.num_vuelo);
...
std::vector<Equipaje> equipajes_para(VueloTrasbordo trasbordo) {
   ...
   int rfid_inicial = trasbordo.vuelo_origen.num_vuelo + trasbordo.vuelo_destino.num_vuelo;

...
int main(int argc, char** argv) {
	...
	if (argc < 6) {
		Log::crit(
				"Insuficientes parametros para robot de carga, se esperaba (id, path_cinta_contenedor, id_cinta_contenedor, path_controlador_carga, path_admin_contenedores)\n");
		exit(1);
	}
	...
	for (;;) {
		...
		while (equipajes_cargados < equipajes_por_cagar) {
			...
			if (equipaje.peso() <= MAX_PESO_CONTENEDOR) {
				...
				Log::crit(
						"RobotCarga(%s) El equipaje %ses mas grande que el propio contenedor!!!\n",

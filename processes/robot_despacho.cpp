...
int main(int argc, char** argv) try {
	...
	Log::info("Iniciando robot despacho(%s), cinta_central a cinta_contenedor:%s\n", argv[1], path_cinta_contenedor);
	...
	for (;;) {
      ...
      if (rfid_equipaje.sospechoso) {
         ...
         Log::info("Robot de despacho(%s) Equipaje %d sospechoso, vuelvo a ponerlo en la cinta central\n",

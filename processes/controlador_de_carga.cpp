...
int main( int argc, char** argv ) {
   ...
   if (argc < 4) {
      Log::crit("Insuficientes parametros contr_de_carga, se esperaba (path_controlador_carga, id_robot_carga num_vuelo)\n");
      exit(1);
   }

   ApiCarga api_carga(argv[1], atoi(argv[2]));
   ...
   if(equipajes_de_vuelo.size() > 0) {
      Log::info("Llego el equipaje completo del vuelo %s", argv[3]);
   } else {

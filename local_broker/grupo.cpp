...
Grupo::Grupo(const std::string & directorio_de_trabajo,
		std::string nombre_recurso) :
		memoria(
				std::string(directorio_de_trabajo).append(PREFIJO_RECURSO).append(
						nombre_recurso).append(".lck").c_str(), char(0), 0,
				false, false), mutex(
				std::string(directorio_de_trabajo).append(PREFIJO_RECURSO).append(
						nombre_recurso).append(".lck").c_str(), char(1), 0), cola(
				std::string(directorio_de_trabajo).append(PREFIJO_RECURSO).append(
						nombre_recurso).append(".lck").c_str(), char(2)), nombre_recurso(
				nombre_recurso) {
	mensaje.tipo = 1;
	...
	for (int i = 1; i < MAX_CLIENTS; i++) {
		client_names[i] = client_names[i - 1]
				+ MAX_NOMBRE_RECURSO * sizeof(char);
	}
	tengo_token = reinterpret_cast<int *>(client_names[0]
			+ MAX_CLIENTS * MAX_NOMBRE_RECURSO * sizeof(char));
...
Grupo::Grupo(const std::string & directorio_de_trabajo,
		std::string nombre_recurso, size_t tamanio_memoria, bool create) :
		memoria(
				std::string(directorio_de_trabajo).append(PREFIJO_RECURSO).append(
						nombre_recurso).append(".lck").c_str(), char(0),
				TAMANIO_TABLA_CLIENTES + tamanio_memoria, 0664, true, false), mutex(
				std::vector<short unsigned int>(1, 1),
				std::string(directorio_de_trabajo).append(PREFIJO_RECURSO).append(
						nombre_recurso).append(".lck").c_str(), char(1), 0664), cola(
				std::string(directorio_de_trabajo).append(PREFIJO_RECURSO).append(
						nombre_recurso).append(".lck").c_str(), char(2), 0664,
				true), nombre_recurso(nombre_recurso) {
	mensaje.tipo = 1;
	if (create) {
		cant_clientes = static_cast<int *>(memoria.memory_pointer());
		mem_size = reinterpret_cast<size_t *>(cant_clientes + 1);
...
void Grupo::join(const char nombre[MAX_NOMBRE_RECURSO]) {
	...
	if (*cant_clientes == MAX_CLIENTS) {
		mutex.signalize(0);
		throw GenericError("Cantidad maxima(%d) de clientes alcanzada",
				MAX_CLIENTS);
	}
	...
	std::cout << "Join: " << nombre << " to " << this->nombre_recurso
			<< std::endl;
...
void Grupo::leave(const char nombre[MAX_NOMBRE_RECURSO]) {
	...
	std::cout << "Leave: " << nombre << " from " << this->nombre_recurso
			<< std::endl;
...
unsigned short Grupo::obtener_numero_cola_de_cliente(
		const char nombre[MAX_NOMBRE_RECURSO]) {
	mutex.wait_on(0);
...
void Grupo::pasar_token_a_proximo_cliente() {
	...
	if (strncmp(client_names[cliente_actual], "localbroker",
			strlen("localbroker")) == 0) {
		*tengo_token = 0;
		*token_enviado = 0;
	} else {
		*tengo_token = 1;
	...
	std::cout << "Passing Token from " << this->nombre_recurso << " to: "
			<< client_names[cliente_actual] << "(" << cliente_actual << ")"
			<< std::endl;
	//}
...
void Grupo::reenviar_token_al_cliente() {
	...
	mensaje.tipo = 1;
	std::cout << "Resending Token from " << this->nombre_recurso << " to: "
			<< client_names[cliente_actual] << "(" << cliente_actual << ")"
			<< std::endl;
...
int Grupo::lock_token() {
	...
	cola.pull(&mensaje, sizeof(traspaso_token_t) - sizeof(long),
			this->numero_cola_asignada);
	return mensaje.tipo;
}
...
void Grupo::release_token(MessageQueue * cola_token_mgr) {
	mensaje.mtype = 1;
	mensaje.tipo = 1;
	strncpy(mensaje.recurso, this->nombre_recurso.c_str(),
			sizeof(mensaje.recurso));
...
void Grupo::replicar_brokers() {
	...
	if (*cant_clientes > 0) {
		...
		for (j = 0; j < MAX_CLIENTS; j++) {
			if (strncmp("localbroker", client_names[j], strlen("localbroker"))
					== 0) {
				encontrado = true;
				break;
			}
		...
		if (encontrado) {
			//if (this->nombre_recurso == "cinta_principal" || this->nombre_recurso.substr(0, 4) == "cpp_") {
			std::cout << "Replicating " << this->nombre_recurso << " to: "
					<< client_names[j] << "(" << cliente_actual << ")"
					<< std::endl;
			//}

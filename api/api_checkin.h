...
typedef struct vuelo_en_checkin {
	...
	vuelo_en_checkin()
		: num_vuelo(-1), cant_equipajes(0), id_cinta_checkin(-1)
	{
	}
	vuelo_en_checkin(int id_cinta_checkin)
		: num_vuelo(-1), cant_equipajes(0), id_cinta_checkin(id_cinta_checkin)
	{
	}
} tVueloEnCheckin;
...
private:
	int id_checkin;
public:
	PuestoCheckinSinVueloAsignado(int id_checkin)
...
public:
	...
	ApiCheckIn(const char* directorio_de_trabajo, const char* config_file, int id_checkin, int id_cinta_checkin,
		bool create);
...
private:
	...
	int id_checkin;
	yasper::ptr< SharedObject<tVueloEnCheckin> > vuelo_actual;
	yasper::ptr< CintaCheckin > cinta_checkin_out;
	yasper::ptr<SemaphoreSet> sem_set;
	yasper::ptr<Mutex> mutex_checkin;

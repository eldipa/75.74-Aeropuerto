...
public:
	CintaPrincipal(const char * absolute_path, int size, int cant_productores, int cant_consumidores);
	CintaPrincipal(const char * absolute_path);
...
template<typename T> CintaPrincipal<T>::CintaPrincipal(const char * absolute_path, int size,
	...
	valores.clear();
	valores.push_back(1);
	for (i = 1; i < cant_consumidores; i++) {
		valores.push_back(0);
	}
	semaforo_consumidores = new SemaphoreSet(valores, absolute_path, 3, 0664);
	...
	vector_elementos = reinterpret_cast<T *>(ids_productores_esperando + cant_productores);
...
template<typename T> CintaPrincipal<T>::CintaPrincipal(const char * absolute_path) {

...
public:
	...
	ApiDespachante(const char * directorio_de_trabajo, const char * nombre_aeropuerto,
...
public:
	AsignacionZona() :
			num_zona(0), num_vuelo(0) {
	}

	AsignacionZona(int zona, int vuelo) :
			num_zona(zona), num_vuelo(vuelo) {
	}
...
public:
	...
	void asignar_vuelo(int zona, int vuelo) {
		...
		Log::info("ApiDespachante zona asignada zona=%d vuelo=%d cant=%d",
				zonas[cant_zonas_asignadas - 1].zona(), zonas[cant_zonas_asignadas - 1].vuelo(),
				cant_zonas_asignadas);
	}
	...
	int get_zona(int num_vuelo) const {
		...
		while ((zona == -1) && (i < cant_zonas_asignadas)) {
			Log::info("ApiDespachante get_zona(num_vuelo=%d): zona=%d vuelo =%d", num_vuelo,
					zonas[i].zona(), zonas[i].vuelo());

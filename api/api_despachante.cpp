...
ApiDespachante::ApiDespachante(const char* directorio_de_trabajo, const char* config_file, const char * nombre_app,
	int numero_despachante, bool con_cinta)
	:
		clnt_torre_de_control(
			clnt_create(ApiConfiguracion::get_torre_ip(config_file).c_str(), TORREDECONTROLPROG, TORREDECONTROLVERS,
				"tcp"))
{
	...
	if (con_cinta) {
		cinta = new CintaCentral(std::string(nombre_app).append(intToString(numero_despachante)).c_str(),
			directorio_de_trabajo, numero_despachante - 1, -1);
	}

	if (clnt_torre_de_control == NULL) {
#if DEBUG_CINTA_PRINCIPAL==0

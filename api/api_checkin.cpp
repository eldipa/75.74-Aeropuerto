...
ApiCheckIn::ApiCheckIn(int id_checkin, const char* path_to_locks, int id_cinta_checkin) :
	...
	sem_set = std::auto_ptr<SemaphoreSet>(
			new SemaphoreSet(path_to_puesto_checkin_lock, id_checkin * cant_ipcs, 1));
	mutex_checkin = std::auto_ptr<Mutex>(new Mutex(*sem_set, 0));
	queue_pasajeros = std::auto_ptr<MessageQueue>(
			new MessageQueue(path_to_puesto_checkin_lock, id_checkin * cant_ipcs + 1));
	vuelo_actual = std::auto_ptr<SharedObject<int> >(
			new SharedObject<int>(path_to_puesto_checkin_lock, id_checkin * cant_ipcs + 2, 0664));
}
